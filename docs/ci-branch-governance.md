...
| pnpm audit (high+)                               | `deploy.yml` + `security.yml` | High/critical only; `check:audit-ignore-paths` fails if a documented GHSA ignore appears outside its allowed non-production path                                                                              |
