...
| **Latest-head review quiescence** | Exact predicate (authoritative source: `.cursor/rules/011-coderabbit-pr-gate.mdc` step 7 — this row restates it, does not vary it): **either (a)** a real, non-`CHANGES_REQUESTED` CodeRabbit review (`APPROVED`/`COMMENTED` with a body) targets the **merge tree SHA**, **or (b)** CodeRabbit has produced no real review after 3 wait/re-trigger cycles or >90m total, **and** Sourcery has a real non-rate-limited review on that SHA with everything resolved, **and** no other in-scope bot has an unresolved item or active `CHANGES_REQUESTED` — **plus, in both cases, all actionable threads (including body-only "outside diff range" findings) are resolved**. Rate-limit placeholders never count as approvals on their own; see `docs/audits/2026-08-03-post-merge-scientific-integrity-review.md` (PR #213) |
