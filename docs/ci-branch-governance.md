...
| **Latest-head review quiescence** | Exact predicate (authoritative source: `.cursor/rules/011-coderabbit-pr-gate.mdc` step 7 — this row restates it, does not vary it). Holds **if and only if all of the following are true**:<br><br>**1. Primary CodeRabbit/Sourcery condition** — either:<br>&nbsp;&nbsp;**(a)** a real, non-`CHANGES_REQUESTED` CodeRabbit review (`APPROVED` or `COMMENTED` with a body) targets the **merge tree SHA**; **or**<br>&nbsp;&nbsp;**(b)** both of the following:<br>&nbsp;&nbsp;&nbsp;&nbsp;- CodeRabbit has produced no real review after **either** 3 failed wait/re-trigger cycles **or** a single wait exceeding 90 minutes; **and**<br>&nbsp;&nbsp;&nbsp;&nbsp;- Sourcery has a real, non-rate-limited review on that SHA with everything resolved.<br><br>**2. No other in-scope bot has an unresolved item or active `CHANGES_REQUESTED`.**<br><br>**3. Every GraphQL thread is resolved.**<br><br>**4. Every body-only finding** (including "outside diff range" comments, which have no thread ID) **has a recorded disposition** of `fixed`, `replied`, or `deferred`. Thread resolution alone does not cover body-only findings.<br><br>Rate-limit placeholders never count as approvals on their own; see `docs/audits/2026-08-03-post-merge-scientific-integrity-review.md` (PR #213) |
