...
### Incident — main tip WebKit cancelled during apt install (2026-08-03)

- **Symptom:** `main` tip `2d8d4f9` (PR10) cross-browser run [`30807274942`](https://github.com/qnbs/AI-Research-Orchestrator/actions/runs/30807274942) concluded **cancelled**; only WebKit cancelled, Firefox/mobile-chrome green.
...
- **Related pattern:** Burst merges also cancelled earlier `main` runs while `cancel-in-progress: true` applied to push events (e.g. `f5b5ed7` cancelled when `4fd9419` pushed) — fixed by PR-only concurrency in PR11.
