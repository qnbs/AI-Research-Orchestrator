...
| Status     | **Partially resolved** (2026-08-06) — verified ruleset snapshot directly against `gh api repos/qnbs/AI-Research-Orchestrator/rulesets/20291814`: `required_review_thread_resolution` and `strict_required_status_checks_policy` were already `true` (no mutation needed; docs were stale, now corrected in `docs/ci-branch-governance.md` and this file's sibling audit doc). CODEOWNERS for critical paths remains open. |
