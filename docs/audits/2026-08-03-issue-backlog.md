...
| Status     | **Resolved** (2026-08-30) — ruleset already had `required_review_thread_resolution` and `strict_required_status_checks_policy`. `.github/CODEOWNERS` routes critical paths to `@qnbs` (#265). Dismiss-stale-approvals-on-new-commits remains maintainer-gated (do not flip; the ruleset still does not require Code Owner reviews). |
