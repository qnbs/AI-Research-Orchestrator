...
UI maps phases to conceptual agents (`QueryGenerator`, `PubMedFetcher`, `ArxivFetcher`, `Ranker`, `Synthesizer`, …) via stable `phaseId` values (`src/types/pipelineEvents.ts`, `src/app/getAgentForPhase.ts`) — **prompt/phase roles**, not separate SDK processes. Free-text phase labels are display-only (ADR 0020).
