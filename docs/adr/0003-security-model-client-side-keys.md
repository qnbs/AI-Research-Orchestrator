...
- **Cross-tab vault initialization is serialized with the Web Locks API** (`navigator.locks.request('ai-research-api-key-vault', …)` around resolve/generate/save in `apiKeyService`). A second tab waits for the first to finish, then re-reads IndexedDB and reuses the saved `CryptoKey` instead of racing a second `generateKey`. Environments without Web Locks fall back to the in-tab promise memoization only — residual risk only on that unsupported path.
