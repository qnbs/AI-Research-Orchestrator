...
export const fetchKnowledgeBase = createAsyncThunk('knowledgeBase/fetchAll', async () => {
...
export const clearKb = createAsyncThunk('knowledgeBase/clearAll', async () => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteKbEntries.fulfilled, (state, action) => {
        ...
        const remainingPmids = new Set<string>();
        state.ids.forEach((id) => {
          state.entities[id]?.articles.forEach((a) => remainingPmids.add(a.pmid));
        });
        state.selectedPmids = state.selectedPmids.filter((pmid) => remainingPmids.has(pmid));
      })
      .addCase(deleteKbEntries.rejected, (state, action) => {
        state.error = action.error.message || 'Failed to delete knowledge base entries';
      })
...
export const { selectAll: selectAllEntries, selectById: selectEntryById } =
...
export const selectUniqueArticles = createSelector([selectAllEntries], (entries) => {
...
export const selectFilteredArticles = createSelector(
  ...
  (articles, filter) => {
    ...
    if (filter.selectedTopics.length > 0)
      result = result.filter((a) => filter.selectedTopics.includes(a.sourceTitle));
    if (filter.selectedTags.length > 0)
      result = result.filter((a) =>
        filter.selectedTags.some((t) => (a.customTags || []).includes(t)),
      );
    if (filter.selectedArticleTypes.length > 0)
      result = result.filter(
        (a) => a.articleType && filter.selectedArticleTypes.includes(a.articleType),
      );
    if (filter.selectedJournals.length > 0)
      result = result.filter((a) => filter.selectedJournals.includes(a.journal));
...
export const selectRecentResearchEntries = createSelector(
  [selectAllEntries],
  (entries) => entries.filter((e): e is ResearchEntry => e.sourceType === 'research').slice(0, 5), // Get top 5
);
