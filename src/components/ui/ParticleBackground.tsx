...
const ParticleBackgroundInner: React.FC<ParticleBackgroundProps> = ({
  ...
  return (
    <div aria-hidden="true" className={`particle-field ${className}`}>
      {particles.map((p) => (
        <motion.span
          key={p.id}
          ...
          style={{
            ...
            background: p.isAccent ? 'var(--color-accent-cyan)' : 'var(--color-brand-accent)',
            boxShadow: `0 0 ${p.size * 2}px var(--color-glow)`,
          }}
          animate={{
            x: [0, p.dxA, p.dxB, 0],
            y: [0, p.dyA, p.dyB, 0],
            opacity: [p.opacity, Math.min(p.opacity * 1.8, 0.7), p.opacity * 0.5, p.opacity],
            scale: [1, 1.3, 0.8, 1],
          }}
          transition={{
            duration: p.duration,
