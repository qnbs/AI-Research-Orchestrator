  ...
  if (nodes.length < 2) {
    return (
      <p className="text-sm text-text-secondary text-center py-10">
        Not enough multi-author articles for a network visualization.
      </p>
    );
  }
    ...
    <div>
      <div className="flex justify-between items-center mb-3">
        <p className="text-xs text-text-secondary">
          {nodes.length} authors · {edges.length} co-authorship links
        </p>
        <button
...
const DashboardViewComponent: React.FC<DashboardViewProps> = ({ onFilterChange, onViewChange }) => {
  ...
  const data = useMemo(() => {
    ...
    const oa = [
      { name: 'Open Access', value: oaCount },
      { name: 'Closed Access', value: articles.length - oaCount },
    ];

    const arxivCount = articles.filter((a) => (a.pmid ?? '').startsWith('arxiv:')).length;
    const source = [
      { name: 'PubMed', value: articles.length - arxivCount },
      { name: 'arXiv', value: arxivCount },
    ].filter((s) => s.value > 0);
    ...
    <div className="space-y-6 pb-8 animate-fadeIn">
      <div className="text-center">
        ...
        <p className="mt-1.5 text-sm text-text-secondary">
          {articles.length.toLocaleString()} articles · interactive charts with zoom &amp; SVG
          export
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <ChartCard
          ...
          subtitle="Drag brush below to zoom"
        ...
        <ChartCard
          ...
          subtitle="Click bar to filter Knowledge Base"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          <ResponsiveContainer width="100%" height={270}>
            <PieChart>
              ...
              <Legend
                iconType="circle"
                iconSize={8}
                formatter={(v) => <span style={{ fontSize: 10, color: '#7d8590' }}>{v}</span>}
              />
            </PieChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard
          ...
          subtitle="PubMed vs arXiv preprints"
          ...
          <ResponsiveContainer width="100%" height={270}>
            <PieChart>
              <Pie
                ...
                labelLine={{ stroke: 'rgba(125,133,144,0.4)' }}
              >
                <Cell fill="#2dd4bf" fillOpacity={0.85} stroke="transparent" />
      ...
      {data.keywords.length > 0 && (
        <ChartCard
          ...
          subtitle={`${data.keywords.length} keywords — cell size = frequency`}
