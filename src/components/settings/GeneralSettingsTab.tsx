...
const LanguageOptionButton: React.FC<{
  lang: Lang;
  badge: string;
  badgeClass: string;
  labelKey: 'settings.language.en' | 'settings.language.de';
  hintKey: 'settings.language.en_hint' | 'settings.language.de_hint';
}> = ({ lang, badge, badgeClass, labelKey, hintKey }) => {
  const { tempSettings, setTempSettings, t } = useSettingsView();
  const selected = tempSettings.appLanguage === lang;
  return (
    <button
      type="button"
      onClick={() => setTempSettings((s) => ({ ...s, appLanguage: lang }))}
      className={`flex-1 p-3 rounded-lg border-2 transition-all flex items-center gap-3 ${selected ? 'border-brand-accent bg-brand-accent/5' : 'border-border hover:border-brand-accent/50'}`}
    >
      <span
        className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${badgeClass}`}
      >
        {badge}
      </span>
      <span className="text-left">
        <span className="block font-medium text-text-primary">{t(labelKey)}</span>
        <span className="block text-xs text-text-secondary">{t(hintKey)}</span>
      </span>
    </button>
  );
};
    ...
    >
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <LanguageOptionButton
          ...
          labelKey="settings.language.de"
          hintKey="settings.language.de_hint"
        />
      </div>
    </SettingCard>
  );
...
const CustomColorField: React.FC<{ colorType: 'primary' | 'secondary' | 'accent' }> = ({
  colorType,
}) => {
  const { tempSettings, setTempSettings, t } = useSettingsView();
  ...
  return (
    <label className="block text-sm font-medium text-text-primary">
      {colorLabel}
      <span className="mt-1 flex items-center gap-2 p-1.5 border border-border rounded-md bg-surface">
        <input
          type="color"
          id={`color-${colorType}`}
...
const ThemePicker: React.FC = () => {
  ...
  const themes = [
    {
      id: 'light' as const,
      aria: t('settings.theme.light_aria'),
      label: t('settings.theme.light'),
      preview: (
        <span className="w-20 h-12 bg-[#eef3f7] rounded-md flex items-center justify-center border border-border">
    ...
    {
      id: 'dark' as const,
      aria: t('settings.theme.dark_aria'),
      label: t('settings.theme.dark'),
      preview: (
        <span className="w-20 h-12 bg-[#070b12] rounded-md flex items-center justify-center border border-border">
    ...
    {
      id: 'matrix' as const,
      aria: t('settings.theme.matrix_aria'),
      label: t('settings.theme.matrix'),
      preview: (
        <span className="w-20 h-12 bg-[#020805] rounded-md flex items-center justify-center border border-success/40">
    ...
    <div className="flex items-center flex-wrap gap-2">
      {themes.map((theme) => (
        <button
          ...
          className={`p-2 rounded-lg border-2 ${tempSettings.theme === theme.id ? 'border-brand-accent' : 'border-transparent'}`}
          aria-label={theme.aria}
        >
          {theme.preview}
          <span className="text-sm mt-1 block text-text-primary">{theme.label}</span>
        </button>
      ))}
    </div>
  );
...
const DensityPicker: React.FC = () => {
  const { tempSettings, setTempSettings, t } = useSettingsView();
  const setDensity = (density: Settings['appearance']['density']) =>
    setTempSettings((s) => ({ ...s, appearance: { ...s.appearance, density } }));
...
const FontFamilyPicker: React.FC = () => {
  const { tempSettings, setTempSettings, t } = useSettingsView();
  return (
    <div className="pt-4 mt-4 border-t border-border">
...
const CustomColorsSection: React.FC = () => {
  const { tempSettings, setTempSettings, t } = useSettingsView();
  return (
    <div className="pt-4 mt-4 border-t border-border">
      <Toggle
...
const PwaInstallPromptBody: React.FC = () => {
  const { handleInstallPwa, t } = useSettingsView();
  return (
    <div>
      <button
        type="button"
...
const PwaUnavailableBody: React.FC = () => {
  const { t } = useSettingsView();
  return (
...
const PwaSettingsCard: React.FC = () => {
  const { isPwaInstalled, installPromptEvent, t } = useSettingsView();
  const body = isPwaInstalled ? (
    <PwaInstalledBody />
  ) : installPromptEvent ? (
...
const NotificationPositionField: React.FC = () => {
  const { tempSettings, setTempSettings, t } = useSettingsView();
  return (
    <label htmlFor="notif-pos" className="block text-sm font-medium text-text-primary mb-1">
      ...
      >
        <option value="bottom-right">{t('settings.notifications.position.bottom_right')}</option>
...
const NotificationDurationField: React.FC = () => {
  const { tempSettings, setTempSettings, t } = useSettingsView();
  return (
