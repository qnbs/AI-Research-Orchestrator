...
export const GeneralSettingsTab: React.FC = () => {
    const { tempSettings, setTempSettings, handleInstallPwa, isPwaInstalled, installPromptEvent, t } = useSettingsView();

    return (
    <div className="space-y-8">
        <SettingCard icon={<GlobeAltIcon className="w-6 h-6 text-brand-accent"/>} title={t('settings.language')} description={t('settings.language.desc')}>
            <div className="flex flex-col sm:flex-row gap-4">
                ...
                >
                    <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-800 flex items-center justify-center font-bold text-xs">EN</div>
                ...
                >
                    <div className="w-8 h-8 rounded-full bg-yellow-100 text-yellow-800 flex items-center justify-center font-bold text-xs">DE</div>
                    <div className="text-left">
        ...
        <SettingCard icon={<SunIcon className="w-6 h-6 text-accent-amber"/>} title={t('settings.appearance')} description="Customize the look and feel of the application.">
        ...
        <SettingCard icon={<InstallIcon className="w-6 h-6 text-brand-accent"/>} title="Application" description="Manage the application's installation status and offline capabilities.">
             {isPwaInstalled ? (
                ...
                <div>
                    <button onClick={handleInstallPwa} className="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-brand-text-on-accent bg-brand-accent hover:bg-opacity-90">
                        <InstallIcon className="h-5 w-5 mr-2" />
                        Install App on this Device
                    </button>
                    <p className="text-xs text-text-secondary mt-2">Install for a native-like experience, including an icon on your home screen or desktop.</p>
                </div>
            ) : (
                <div className="flex items-center gap-3 text-text-secondary text-sm">
                    <InfoIcon className="h-5 w-5 flex-shrink-0" />
                    <span>This app can be installed on your device. Your browser may offer an install option in the address bar or settings menu.</span>
                </div>
            )}
        </SettingCard>
            ...
            <SettingCard icon={<SparklesIcon className="w-6 h-6 text-accent-magenta"/>} title="AI Configuration" description="Fine-tune the AI's behavior, language, and core instructions.">
                <div className="space-y-6">
                    <div>
                        ...
                        <select id="ai-model" value={tempSettings.ai.model} onChange={(e) => setTempSettings(s => ({...s, ai: {...s.ai, model: e.target.value as any}}))} className="mt-1 block w-full bg-input-bg border border-border rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-brand-accent">
                        ...
                        <p className="text-xs text-text-secondary mt-1">Use Gemini 2.5 Flash for speed and efficiency, or Gemini 3 Pro for complex reasoning tasks.</p>
                    </div>
                    ...
                    <div>
                        <label htmlFor="ai-language" className="font-medium text-text-primary">AI Language (Output)</label>
                        ...
                        <p className="text-xs text-text-secondary mt-1">Controls the language of generated reports, independent of the interface language.</p>
                    </div>
...
export const ExportSettingsTab: React.FC = () => {
    const { tempSettings, setTempSettings, handleSelectAllCsvColumns, handleDeselectAllCsvColumns } = useSettingsView();
...
export const DataSettingsTab: React.FC = () => {
    const { storageUsage, handleExportHistory, handleExportKnowledgeBase, fileInputRef, handleExportSettings, settingsFileInputRef, setModalState, knowledgeBase, uniqueArticles, handleImport, handleImportSettings } = useSettingsView();
    ...
    <div className="space-y-8">
        ...
        <SettingCard title="Data Backup & Restore" description={`You have ${knowledgeBase.length} reports containing ${uniqueArticles.length} unique articles.`}>
            <div className="space-y-3">
