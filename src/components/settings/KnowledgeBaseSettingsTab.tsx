...
const MergeDuplicatesButton: React.FC = () => {
...
const PruneByScoreButton: React.FC = () => {
  const { setModalState, t } = useSettingsView();
  return (
    <button
      type="button"
...
const PresetRow: React.FC<{ id: string; name: string }> = ({ id, name }) => {
  const { setModalState, t, presets } = useSettingsView();
  ...
  return (
    <div className="flex items-center justify-between p-2 bg-surface rounded-md border border-border">
      ...
      <button
        ...
        aria-label={t('settings.kb.presets.delete_aria', { name })}
      >
...
const PresetsCard: React.FC = () => {
  const { presets, t } = useSettingsView();
  return (
