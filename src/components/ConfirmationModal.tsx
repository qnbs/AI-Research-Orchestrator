...
export const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="bg-surface rounded-lg border border-border shadow-2xl w-full max-w-md m-4"
        role="dialog"
        aria-modal="true"
        aria-labelledby="confirmation-modal-title"
      >
        <div className="p-4 flex justify-between items-center border-b border-border">
          <h3 id="confirmation-modal-title" className={`text-lg font-bold ${titleClass}`}>
            {title}
          </h3>
          <button
            onClick={onCancel}
