...
export const useKnowledgeBaseLogic = (
  ...
  const [sortOrder, setSortOrder] = useState<'relevance' | 'newest'>(
    settings.knowledgeBase.defaultSort,
  );
  ...
  const { topics, tags, articleTypes, journals } = useMemo(() => {
    const topics = [
      ...new Set(knowledgeBase.filter((e) => e.sourceType === 'research').map((e) => e.title)),
    ];
  ...
  const filteredArticles = useMemo(() => {
    ...
    if (filter.selectedTopics.length > 0)
      articles = articles.filter((a) => filter.selectedTopics.includes(a.sourceTitle));
    ...
    if (filter.selectedJournals.length > 0)
      articles = articles.filter((a) => filter.selectedJournals.includes(a.journal));
    if (filter.showOpenAccessOnly) articles = articles.filter((a) => a.isOpenAccess);

    return articles.sort((a, b) =>
      sortOrder === 'newest'
        ? new Date(b.pubYear).getTime() - new Date(a.pubYear).getTime()
  ...
  const findRelatedInsights = useCallback(
    (pmid: string) =>
      knowledgeBase
        .flatMap((e) => (e.sourceType === 'research' ? e.report.aiGeneratedInsights || [] : []))
      ...
      try {
        ...
        switch (showExportModal) {
          case 'pdf':
            ...
            break;
          case 'csv':
            exportToCsv(articlesToExport, 'kb_selection', settings.export.csv);
            break;
          case 'bib':
          case 'ris':
