    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <span className="truncate pr-2">
          {title}{' '}
          {selected.length > 0 && (
            <span className="ml-1 bg-brand-accent text-brand-text-on-accent text-xs font-bold rounded-full px-2 py-0.5">
              {selected.length}
            </span>
          )}
        </span>
        <ChevronDownIcon
          className={`w-5 h-5 transition-transform duration-200 shrink-0 ${isOpen ? 'rotate-180' : ''} text-text-secondary`}
        />
      </button>
      {isOpen && (
        <div className="absolute z-10 mt-1 w-full bg-surface border border-border rounded-md shadow-lg max-h-60 overflow-y-auto">
          ...
          {/* eslint-disable-next-line jsx-a11y/no-noninteractive-element-to-interactive-role -- ul role="listbox" + li role="option" is the WAI-ARIA APG's own recommended pattern for building an accessible custom listbox from list markup. */}
          <ul role="listbox" className="py-1">
            {filteredOptions.map((option) => (
              // eslint-disable-next-line jsx-a11y/no-noninteractive-element-to-interactive-role -- see listbox justification above.
    ...
    <aside className="w-64 flex-shrink-0 space-y-4 sticky top-24 self-start hidden md:block">
      ...
      <label className="flex items-center cursor-pointer group">
        <div className="relative">
          <input
            type="checkbox"
            checked={filter.showOpenAccessOnly}
            onChange={(e) => onFilterChange({ showOpenAccessOnly: e.target.checked })}
            className="sr-only"
          />
          <div
            aria-hidden="true"
            className={`w-5 h-5 rounded-md border-2 transition-all duration-200 flex items-center justify-center group-hover:border-brand-accent ${filter.showOpenAccessOnly ? 'bg-brand-accent border-brand-accent' : 'bg-surface border-border'}`}
          >
...
export const BulkActionsToolbar: React.FC = () => {
  ...
  return (
    <div
      className="sticky top-[112px] z-10 bg-surface/80 backdrop-blur-md p-3 rounded-lg border border-border mb-4 flex justify-between items-center shadow-lg animate-fadeIn"
...
const KBArticleCard: React.FC<{
  ...
  return (
    <div
      className={`bg-surface border rounded-lg flex flex-col justify-between transition-all duration-200 hover:shadow-xl hover:-translate-y-1 text-left group ${isSelected ? 'border-brand-accent shadow-lg ring-2 ring-brand-accent/50' : 'border-border'} ${densityClasses.container}`}
    >
      <div>
        ...
        <p className={`text-text-secondary mb-3 ${densityClasses.text}`}>
          {article.authors} -{' '}
...
const KBArticleListItem: React.FC<{
  article: AggregatedArticle;
  isSelected: boolean;
  onSelect: (pmid: string) => void;
  onView: (article: AggregatedArticle) => void;
    ...
    >
      ...
      <div className="flex-1">
        <button
          type="button"
          onClick={() => onView(article)}
          className="font-semibold text-text-primary text-left group-hover:text-brand-accent"
        >
          {article.title}
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {paginatedArticles.map((a) => (
          <KBArticleCard
            key={a.pmid}
            article={a}
            isSelected={selectedPmids.includes(a.pmid)}
