    ...
    <div className="max-w-5xl mx-auto animate-fadeIn space-y-8 pt-2">
      <div className="text-center">
        ...
        <p className="mt-2 text-lg text-text-secondary">
          Discover, analyze, and track scientific journals.
        </p>
      </div>

      <form
        onSubmit={handleAnalyzeJournal}
        className="bg-surface p-6 rounded-lg border border-border shadow-lg space-y-6"
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label htmlFor="topic" className="block text-sm font-semibold text-text-primary mb-2">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-2">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full sm:w-auto inline-flex justify-center items-center py-2.5 px-6 border border-transparent shadow-sm text-sm font-semibold rounded-md text-brand-text-on-accent bg-brand-accent hover:bg-opacity-90 disabled:opacity-50 transition-all hover:shadow-brand-accent/20 hover:shadow-lg"
          >
            {isLoading ? (
      ...
      {error && !isLoading && (
        <div className="text-center text-red-400 p-4 bg-red-500/10 border border-red-500/20 rounded-md animate-fadeIn">
          {error}
        </div>
      )}
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {journalProfile.focusAreas.map((area) => (
                    <span
                      key={area}
                      className="bg-surface-hover text-text-primary text-xs font-medium px-3 py-1 rounded-full border border-border"
                    >
                      {area}
                    </span>
                  ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-surface p-6 rounded-lg border border-border flex flex-col">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-xs font-medium bg-brand-accent text-white px-2 py-1 rounded-full">
