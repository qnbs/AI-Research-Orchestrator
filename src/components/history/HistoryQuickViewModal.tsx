...
const QuickViewHeader: React.FC<{
  typeTitle: string;
  title: string;
  onClose: () => void;
}> = ({ typeTitle, title, onClose }) => {
  const { t } = useTranslation();
  return (
...
const QuickViewStats: React.FC<{
  foundLabel: string;
  articleCount: number;
  keywords: string[];
}> = ({ foundLabel, articleCount, keywords }) => {
  const { t } = useTranslation();
  return (
    <div className="space-y-4 my-6">
      <div>
        ...
        <p className="text-text-primary font-medium">{articleCount}</p>
      </div>
      <div>
        <h4 className="text-xs font-semibold text-text-secondary uppercase tracking-wider">
          {t('history.quick.top_keywords')}
        </h4>
        <QuickViewKeywords keywords={keywords} />
      </div>
    </div>
  );
};

const QuickViewActions: React.FC<{
  viewAction: string;
  onClose: () => void;
  onOpen: () => void;
}> = ({ viewAction, onClose, onOpen }) => {
  const { t } = useTranslation();
  return (
    ...
    >
      ...
      >
        ...
        <QuickViewStats
          ...
          keywords={keywordsAndConcepts}
        />
        <QuickViewActions viewAction={viewAction} onClose={onClose} onOpen={openFullEntry} />
      </div>
    </div>
  );
};
