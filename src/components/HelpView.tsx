...
const HelpView: React.FC<HelpViewProps> = ({ initialTab, onTabConsumed }) => {
    ...
    const faqItems = useMemo(() => [
        { title: "Is my data private?", content: (<><p><strong>Yes.</strong> All data, including your research history, saved articles, and settings, is stored exclusively in your browser's <code>localStorage</code>. No information is ever uploaded to a server or shared. Your research is completely private to the browser you are using.</p><Note type="warning" title="Back Up Your Data">Because the data is stored locally, it can be lost if you clear your browser's data. Use the export features in <code>Settings → Data Management &amp; Privacy</code> regularly to create JSON backups.</Note></>) },
