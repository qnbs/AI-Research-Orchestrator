...
import { ReportAccordionSection } from './report-display/ReportAccordionSection';
import { ReportArticleCard } from './report-display/ReportArticleCard';
import { secureMarkdownToHtml, stripMarkdown } from './report-display/reportDisplayUtils';
...
export const ReportDisplay: React.FC<ReportDisplayProps> = React.memo(function ReportDisplay({
  ...
  const handleCopySynthesis = useCallback(() => {
    ...
    navigator.clipboard.writeText(plainText).then(() => {
      setNotification({
        id: Date.now(),
        message: 'Synthesis (plain text) copied to clipboard!',
        type: 'success',
      });
    });
    ...
    <>
      <div className="animate-fadeIn bg-surface rounded-lg border border-border flex flex-col shadow-lg">
                ...
                {isExportMenuOpen && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full text-left flex items-center gap-3 px-3 py-2 text-sm hover:bg-surface-hover"
                    >
                      <SparklesIcon className="h-4 w-4" /> Insights CSV
                    </button>
                  </div>
                )}
              </div>
        ...
        <div className="flex-grow">
          ...
          <ReportAccordionSection title="Data Visualization & Trends" defaultOpen={false}>
            <div className="p-2">
              {chartData ? (
                <div className="h-64 bg-background border border-border rounded-md p-4">
                  <Bar data={chartData.data} options={chartData.options as ChartOptions<'bar'>} />
                </div>
              ) : (
          ...
          <ReportAccordionSection title={`Ranked Articles`} count={report.rankedArticles.length}>
            <div className="space-y-4">
          ...
          <ReportAccordionSection
            title="Generated PubMed Queries"
