...
describe('CircuitBreaker', () => {
  ...
  it('recordSuccess while closed resets the failure counter', () => {
    ...
    breaker.recordFailure();
    breaker.recordSuccess();
    // A single subsequent failure should not open the circuit since the
    // counter was reset by the intervening success.
    breaker.recordFailure();
    expect(breaker.getState()).toBe('closed');
  });
  ...
  it('getCircuitBreaker returns the same instance for a given service', () => {
    ...
    // Options from the second call are ignored since the breaker already existed.
    first.recordFailure();
