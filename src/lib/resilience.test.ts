...
describe('cost estimators', () => {
  ...
  it('rounds token estimate up to the nearest whole token', () => {
    expect(estimateTokensFromText('a')).toBe(1); // ceil(1/4)
    expect(estimateTokensFromText('abcde')).toBe(2); // ceil(5/4)
    expect(estimateTokensFromText('abcdefgh')).toBe(2); // ceil(8/4)
  });
});
