...
**This closes the prior audit's single biggest open item (RS-2/the nonAi-integration deferral) outright** — the engine is no longer "isolated, not wired": its functions are called directly from `geminiService.ts`, surfaced in Settings, driven through orchestration, and rendered in the Knowledge Base/Journals/Authors views, with 97.99%/85.03%/94.31% (lines/branches/functions) test coverage. (This direct-call wiring is distinct from the registered `providers/heuristic.ts` **adapter** — see Residual risks — which remains unreachable in practice because `geminiService.ts` resolves to the direct calls before the provider registry is ever consulted; the engine's reachability and that one adapter's reachability are two different facts.)
