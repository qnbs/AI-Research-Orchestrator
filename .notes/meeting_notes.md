...
=======
- **#78 / #74 — DONE on branch `cursor/app-tsx-decomposition-aa80`.** `src/App.tsx` reduced from 869 → ~20 lines via pure structural split: `src/app/useAppLogic.ts` (state/effects/handlers), `src/app/AppLayout.tsx` (chrome + view routing), plus `getAgentForPhase` / spinners / lazyViews. Vault-reset listener semantics preserved.
...
>>>>>>> cac0520 (refactor(app): split App.tsx below 700-line hard max (#78, #74))
