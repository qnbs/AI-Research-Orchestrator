...
- **Synthetic demo quarantine (P0):** Non-AI research no longer silently substitutes the educational demo corpus on empty/failed/offline retrieval. Explicit Educational Demo mode always routes through the Non-AI engine (never a live provider); reports stamp `sourceClass` / `corpusClass`, use Demo ID labels, refuse `verified` trust, and watermark exports (ADR 0016, Dexie v6 with array-safe rankedArticles + checkpoint trust demotion). Empty-retrieval synthesis is preserved through session finalization (`synthesisChunk` + report seed).
