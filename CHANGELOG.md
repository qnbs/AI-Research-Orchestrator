...
- **TypeScript 6 (`NOW-P1-TS6` / #274):** Dev compiler `~6.0.3`. Workspace `peerDependencyRules.allowedVersions.typescript` updated from `5.8.3`. `tsconfig.json` already had `strict: true` and explicit ES2022/`bundler` settings, so no compiler-option ADR. `typescript-eslint` 8.69 peer range is `>=4.8.4 <6.1.0`. Not TypeScript 7.
...
- **Partial CSV/insights watermark (`NOW-P1-PARTIAL-EXPORT-GAPS`):** Report CSV and insights CSV prepend the same `PARTIAL REPORT — RESEARCH DID NOT FINISH` provenance line used on narrative export when `completionStatus === 'partial'`. Spreadsheet downloads of a cancelled run are no longer silent. Playwright cancel-mid-stream coverage hangs Gemini synthesis SSE after ranking (`NOW-P0-PARTIAL-E2E`).
