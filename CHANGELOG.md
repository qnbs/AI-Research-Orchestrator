...
- **`browserslist` high advisories (`NOW-P0-AUDIT-BROWSERSLIST`):** Workspace override pins `browserslist@4.28.8` (patched ≥4.28.7) so `pnpm audit --audit-level=high` stays green. Covers GHSA-c83g-rgw3-j3cx (unbounded memory) and GHSA-73wf-gq98-2v4g (prototype write via custom stats). Paths are dev-only (`autoprefixer`, `eslint-plugin-react-hooks` → `@babel/core`); this is a pin, not a GHSA ignore.
