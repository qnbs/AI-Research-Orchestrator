...
- **PR `edited` cancels security audit:** `security.yml` listens to all `pull_request` types. Patching the PR title or body while it is in flight retriggers concurrency and cancels `pnpm audit (high+)`. Do not update the PR description until that job is terminal (PR #302).
