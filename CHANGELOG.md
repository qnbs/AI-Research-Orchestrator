...
- **Copilot follow-up after #294:** Bottom-nav More stays mounted; the sheet is open only while `openedFor === currentView` (no remount, no `setState` during render). Ollama status ignores leftover cloud-provider default models. Hydration uses the selected provider’s `defaultModel` when the stored model is foreign.
