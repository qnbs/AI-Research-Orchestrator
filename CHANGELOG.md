...
- **pnpm audit retry:** `deploy.yml` and `security.yml` retry `pnpm audit --audit-level=high` up to three times so an npm advisory-bulk timeout (error 23) does not fail the required check.
