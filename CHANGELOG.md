...
- **Heuristic honesty (`NOW-P2-HEURISTIC-UX` / `RANK` / `QUERY` / `SYNTH`):** Adapter synthesis copy names BM25+ lexical / relative 0–100 ranks instead of “semantic ranking”. Non-demo template Background states extractive-template (not a live-model draft); report markdown keeps `##` section headings. Query builder tokenizes EN+DE stopwords and maps high-value German lay terms (Krebs, Bluthochdruck, Schlaganfall, Herzinfarkt, …) onto the existing compact MeSH dictionary. Status/glossary/cost already teach “heuristic is active, not broken AI”.
...
- **Ollama diagnostics + privacy (`NOW-P2-OLLAMA-DIAG` / `NOW-P2-OLLAMA-PRIVACY` / `NOW-P2-OLLAMA-BUDGET`):** Settings health failures teach CORS/loopback vs timeout vs unavailable (no raw `cors`/`timeout` codes). Local AI base-URL help is loopback-first (remote LAN is not first-class). The Orchestrator/Quick-research status line keeps a persistent PubMed/arXiv network note when live Ollama is selected; loopback copy says generation stays on this machine, approved remote endpoints say generation is sent to that Ollama host. The health panel shows the estimated prompt-input budget when a model is selected.
- **Late prompt merge-gate:** `docs/prompts/2026-09-03-late-cursor-grok-audit-perfection-master-prompt.md` §0.2 and Appendix C match live `011` clause **(d)** (CodeRabbit rate-limit is not a hard merge blocker). The archived “wait 3 cycles / 90 minutes” merge instruction is removed so agents do not treat the snapshot as current law. CodeAnt raised this on #303 after squash-merge.
- **Theme visual QA (`NOW-P1-THEME-QA`):** PR template and `CONTRIBUTING.md` include a manual dark / light / matrix checklist for chrome, overlays, and charts (plus accessible table twins). `pnpm run check:contrast` is required when tokens, chrome colors, or overlay/glass colors change; not required for chart-only changes. N/A is exclusive of the surface checks. This PR adds no screenshot suite to CI.
- **360px bottom nav (`NOW-P1-MOBILE-360`):** Five mobile items use short visible labels (Review / Assistant / Library / Explore / More) with full destination names in `aria-label` and `title`. Muted Library still exposes Knowledge Base as the accessible name plus the requires-report hint. Drops the 10px font hack and label truncate; the primary row uses `text-xs` and `overflow-x-hidden`. The fixed `<nav>` does not clip the More sheet. Playwright covers 360px no-horizontal-scroll on the primary row, the five accessible names, and More open/Escape.
