...
- **Copilot follow-up after #294:** Bottom-nav More stays mounted. The sheet is open only while `openedFor === currentView`; a destination change clears `openedFor` so returning to that view does not reopen it. Ollama status ignores leftover cloud-provider default models. Hydration uses the selected provider’s `defaultModel` when the stored model is foreign.
