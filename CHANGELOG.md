...
- **Ollama diagnostics + privacy (`NOW-P2-OLLAMA-DIAG` / `NOW-P2-OLLAMA-PRIVACY` / `NOW-P2-OLLAMA-BUDGET`):** Settings health failures teach CORS/loopback vs timeout vs unavailable (no raw `cors`/`timeout` codes). Local AI base-URL help is loopback-first (remote LAN is not first-class). The Orchestrator/Quick-research status line keeps a persistent PubMed/arXiv network note when live Ollama is selected. The health panel always shows the estimated prompt-input budget and how it was derived.
