...
- **Cancelled reports never marked `done` (P0 scientific integrity, ADR 0021, #234):** Cancelling a running research pipeline with a partial report already collected previously fell through to `reportStatus: 'done'` — identical to a normally completed report, skipping all success-path finalization (provenance stamping, claim extraction, grounded-synthesis assessment). A cancelled, incomplete report could be saved, exported, or chatted against as if it were finished. New `ReportStatus.partial` + `ResearchReport.completionStatus`/`cancelledAtPhase`/`cancelledAt` marker (unified `ReportStatus` type, replacing three duplicated inline unions) is stamped at both cancellation sites (live abort and checkpoint restore) and survives save/export/reopen. `ReportDisplay` shows a non-dismissable partial-report banner and disables the report-chat accordion (which would otherwise silently drop messages); `sanitizeReportForExport` prepends a `PARTIAL REPORT` watermark to exported synthesis text.
- **Sticky settings action bar mobile overlap (#230 post-merge):** A sticky settings header tracking the exact measured chrome height could overlap page content on narrow mobile viewports once a third fixed/sticky element (bottom nav) entered the layout. Now sticky only from `md:` up, with a CSS `max()` floor (`top-[max(9rem,var(--chrome-height,0px))]`) so it only grows past the static breakpoint value when a banner genuinely makes the fixed chrome taller.
- **Status banners rendering behind the fixed header (#228):** `OfflineBanner`/`DemoDataBanner`/`UpdateAvailableBanner`/`ServiceWorkerRegistrationFailedBanner` could render partially hidden under the fixed header on narrow viewports or when multiple banners stacked. `useElementHeight` (`ResizeObserver`-backed) measures the real chrome height and drives `<main>`'s `padding-top` dynamically instead of a static Tailwind breakpoint guess.
- **Navigation gaps and canonical product naming (#229):** Closed keyboard/mobile nav-overflow gaps in `BottomNavBar`/`Header`; unified the product name across UI copy, `manifest.json`, and README to a single canonical form; extracted `orchestratorTranslations.ts`/`helpTranslations.ts` from the monolithic `translations.ts`.
...
- **Cancel/Stop control for running research (#231):** A visible Cancel button while the pipeline is `generating`/`streaming`, wired through `LoadingIndicator`'s new opt-in, caller-labeled `cancel` prop. Aborting persists a resumable checkpoint of whatever was collected so far.
- **Accessible data-table fallback for color-only-encoded charts (#232):** New shared `ChartAccessibleTable` component (extracted from Dashboard's existing pattern) gives every pie/treemap chart a screen-reader-only tabular equivalent — previously only Dashboard's 2 bar charts had one.
- **`prefers-reduced-motion` support for looping Framer Motion animations (#233):** New `useMotionSafeLoop` hook collapses infinite `repeat: Infinity` loops to their resting keyframe (preserving non-duration transition properties like `ease`) when reduced motion is requested; `LoadingIndicator`'s SVG SMIL spinner (invisible to CSS media queries and Framer Motion alike) is separately gated the same way.
- **Sticky settings action bar + developer-mode gating for Agent Debugger (#230):** Settings' title/Save/Cancel bar stays visible while scrolling a long tab (desktop only, see Fixed); the Agent Debugger toggle and panel are now gated behind Settings' `developerMode` flag.
