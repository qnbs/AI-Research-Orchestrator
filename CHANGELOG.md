...
- **Copilot follow-up after #294:** Bottom-nav More remounts on `currentView` change so the sheet starts closed (no `setState` during render or in an effect). Ollama status uses `getProviderMeta('ollama').defaultModel` instead of a hard-coded `local` fallback.
