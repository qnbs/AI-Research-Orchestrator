...
- **Copilot follow-up after #294:** Bottom-nav More no longer calls `setState` during render; it closes in a `useEffect` when `currentView` changes. Ollama status uses `getProviderMeta('ollama').defaultModel` instead of a hard-coded `local` fallback.
