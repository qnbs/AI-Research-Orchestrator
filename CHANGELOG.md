...
- **Audit docs truth (2026-09-02):** Live `main` after the 2026-09-01 wave is `660cf8c` (`v0.4.2` plus the 2026-09-01 wave and #287). Execution planning is `docs/prompts/2026-09-02-cursor-grok-audit-remediation-master-prompt.md`. Evidence record is `docs/audits/2026-09-02-baseline.md`. The 2026-09-01 / 2026-08-02 prompts stay historical. ADR 0010 dated deferral, façade split, and remaining chart a11y are **#290 / this Unreleased set**, not yet on `main`. Heuristic + Ollama remain the zero-cost paths; OpenRouter stays `openai` + custom base URL.
...
- **PDF footer page labels / TOC pages:** Footers are stamped once at save with the page index (`Page 1`, `Page 2`, …), not the final page number on every page. TOC entries are recorded after the section-title page break so a title near the bottom does not point at the previous page.
