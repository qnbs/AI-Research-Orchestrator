...
2. **Always** comment `@deepsourcebot review` on the PR after open and after each fix push (DeepSource AI Review is on-demand for this team — static analysis alone is not enough).
3. Address **every** open inline thread from CodeRabbit, CodeAnt, Copilot, DeepSource AI Review (and any other bot reviewers listed in `.cursor/rules/013-pr-review-correction-loop.mdc`).
4. Read latest bot **review summaries** — CodeRabbit out-of-diff items often appear only in the review body.
5. Reply on each thread (cite fix commit) and **resolve** the conversation.
6. Re-poll after bots finish on the new commit; wait through CodeRabbit rate limits if needed; re-trigger `@deepsourcebot review` after material pushes.
7. Merge only when CI is green and no new actionable bot comments remain.
