...
- **E2E (Playwright)**: blocking Chromium job in `.github/workflows/e2e.yml` runs seven specs: `smoke.spec.ts`, `agent-flow.spec.ts`, `dialog-a11y.spec.ts`, `keyboard-focus.spec.ts`, `skip-to-content.spec.ts`, `journal-hub.spec.ts`, `provider-flow.spec.ts`. Separate blocking `a11y.spec.ts` in `a11y.yml`. Cross-browser matrix (`e2e-cross-browser.yml`) is also **blocking** (Firefox / WebKit / mobile Chrome, same seven specs, `fail-fast: false`, no `continue-on-error`). Config auto-starts the Vite dev server on port 3000 and uses a fake Gemini key; prefers stable `getByRole` selectors; `sleep` only with justification. One-time setup: `pnpm exec playwright install chromium`.
